...
import {HtmlEntityMap, AsciiEmojiMap, ExtendedEmojiMap} from './text-entity-map.js';
...
Tools.getChildrenFromUu5String = function (uu5String, opt) {
  ...
  if (!matchS)
    pointer.content.push(Tools.decodeTextEntity(uu5String));
  else
  // skip <uu5string />
  if (matchS[2] === 'uu5string' && matchS[4] === '/>') {
    if (matchS[5]) pointer.content.push(Tools.decodeTextEntity(matchS[5]));
    pIndex = matchS[0].length;
    matchS = uu5stringRe.exec(uu5String);
  }

  while (matchS) {
    cIndex = matchS.index;
    if (cIndex > pIndex) pointer.content.push(
      pre ? uu5String.substring(pIndex, cIndex) : Tools.decodeTextEntity(uu5String.substring(pIndex, cIndex))
    );
    let childTag = matchS[2];
    let tagObj;

    if (pre && !(childTag === 'uu5string.pre' && matchS[1] === '</')) {
      pointer.content.push(matchS[0]);
    } else {
      ...
      if (matchS[1] === '</') {
        ...
        if (pre) {
          pre = false;
          pointer.content[pointer.content.length - 1] = Tools.decodeHtmlEntity(tagObj.content.join(''));
        } else {
          if (tagObj.forbidden) {
...
Tools.execMetaTag = (tag, args) => {
  //TODO implement each metqTag as separate function, call functions dynamically (witch safety keyword guard)
...
Tools.decodeHtmlEntity = (text, htmlEntityMap = HtmlEntityMap) => {
  const reg = new RegExp(`(\\\\)?${htmlEntityMap.re.source}`, 'g');

  return text.replace(reg, (match0, match1, match2) => match1 ? match2 : htmlEntityMap.items[match2]);
};

Tools.decodeTextEntity = (text) => {
  return UU5.Environment.textEntityReplace ? Environment.textEntityMap.replace(text) : text; // eslint-disable-line no-undef
};

Tools.decodeEmoji = (text, emojiMap = AsciiEmojiMap) => {
  const reg = new RegExp(`(\\\\)?${emojiMap.re.source}`, 'g');

  return text.replace(reg, (match0, match1, match2) => match1 ? match2 : emojiMap.items[match2]);
};
...
Tools.mergeEnvironmentUu5DataMap = function (uu5DataMap) {
  UU5.Environment.uu5DataMap = UU5.Common.Tools.merge(UU5.Environment.uu5DataMap || {}, uu5DataMap); // eslint-disable-line no-undef
};
